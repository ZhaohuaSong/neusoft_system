        ...
        constructor: c, destroy: function () {
            ...
            e.removeClass("hide"), this.options.dataSource(f ? f : {}, function (f) {
                e.addClass("hide"), a.each(f.data, function (e, f) {
                    var g;
                    if ("folder" === f.type) {
                        g = c.$element.find("[data-template=treebranch]:eq(0)").clone().removeClass("hide").removeAttr("data-template"), g.data(f), g.find(".tree-branch-name > .tree-label").html(f.text || f.name),g.find(".tree-branch-name ").append('<span id="id_parent_node" style="display: none">'+f.additionalParameters.id+'</span>');
