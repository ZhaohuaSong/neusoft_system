...
class QUpdateView(UpdateView):
    ...
    #抽象出该方法，当不适用modelform的时候可以在子类去重实现
    def save(self, form):
        self.object = form.save()

    #重新实现GET请求方法，并初始化form各个字段的值，该方法必须form字段和model字段一致
    def get_form(self, data=None, files=None, **kwargs):#obj是model对象
        obj = kwargs['instance']
        #遍历出model所有字段以及值
        params = obj._meta.fields
        dic = {}
        for t in range(len(params)):
            field_name = params[t].name
        ...
        for field_name in form_fields.keys():
            try:
                form.fields[field_name].initial = dic[field_name]
            except KeyError:
                pass
