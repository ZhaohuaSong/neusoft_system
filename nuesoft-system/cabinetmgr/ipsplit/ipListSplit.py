...
class Split:
    ...
    def getIp(self,ip):
        ...
        nu_list = list(range(start, end+1))
        return nu_list
    def getAllIp(self, ip):
        ipRangeList = [self.ipToNum(x) for x in ip.split('-') ]
        start,end = ipRangeList[0],ipRangeList[-1]
    ...
    def getAllIpList(self):
        ...
        ip_list = []
        for element in self.deviceList:
            ip_list.extend(self.getAllIp(element))
        return ip_list

    def ipListToPack(self, num_list):
        ip_list = []
        fun = lambda x: x[1]-x[0]
        for k, g in groupby(enumerate(num_list), fun):
            l1 = [j for i, j in g]
            if len(l1) > 1:
                scop = str(self.numToIp(min(l1))) + '-' + str(self.numToIp(max(l1)))
                ip_list.append(scop)
            else:
                scop = str(self.numToIp(l1[0]))
                ip_list.append(scop)
        return ip_list
