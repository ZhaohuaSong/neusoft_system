...
// Qiailin Datatables 初始化页面 关键绑定 ID (dynamic-table)
    var pospDataTable = $('#dynamic-table').DataTable({
            ...
            "ajax": {
                ...
                "data": function (d) {
                    return $.extend({}, d, {
                      "resource_id": $('#sech_item').attr('role'),
                    });
                }
            },
            ...
            "fnRowCallback": function (nRow, aData, iDataIndex) {
                $('td:eq(-1)', nRow).html('<div class="hidden-sm hidden-xs action-buttons"> '
                    + '<a class="green" href="/catalog/config/list/?resource_id=' + aData[2] + '">'
                    ...
                    + '<li><a href="/catalog/config/list/?resource_id=' + aData[2] + '" class="tooltip-success" data-rel="tooltip" title="" data-original-title="Edit">'
                    + '<span class="green">'
                    + '<i class="ace-icon fa fa-pencil-square-o bigger-120"></i>'
                    + '</span>详情</a></li></ul></div></div>'
                );
                return nRow;
            },
            //
   ...
   //监听输入的回车事件
    ...
    $('#in-search').bind('keypress', function (event) {
        if (event.keyCode == "13") {
            ...
            console.info(insearch)
            if (insearch == null || insearch == '') {
                $('#sech_item').attr('role', '')
            } else {
                $('#sech_item').attr('role', '-11')
            }
            //merchanttreedraw();
    ...
    $(document).ready(function () {
        $.fn.zTree.init($("#treeDem"), setting2);
        //$("#expandAllBtn").bind("click", expandAll);
        //$("#asyncAllBtn").bind("click", asyncAll);
        //$("#resetBtn").bind("click", reset);
    });
    ...
    function onClick(event, treeId, treeNode, clickFlag) {
        $('#sech_item').attr('role', '')
        $('#sech_it').attr('role', '')
        $('#sech_item').attr('role', treeNode.resource_id)
        $('#sech_it').attr('role', treeNode.field_id)
        $('#sech_it_search1').attr('role', treeNode.resource_id)
        $('#sech_it_search2').attr('role', treeNode.field_id)
        //console.info(treeNode.field_id, treeNode.resource_id)
        //console.info(treeNode.resource_id)
        //alert(treeNode.id)
        pospDataTable.search('').draw();
        //pospDataTable.clearPipeline().draw();
    }
    ...
    function updateNodes(highlight, nodeList) {
     ...
     $("#lb_tree_expand").bind("click",
        function () {
            var zTree = $.fn.zTree.getZTreeObj("treeDemo");
            zTree.expandAll(true);

            $("#lb_tree_collapse").show();
            $("#lb_tree_expand").hide();
        });

    $("#lb_tree_collapse").bind("click",
        function () {
            var zTree = $.fn.zTree.getZTreeObj("treeDemo");
            zTree.expandAll(false);

            $("#lb_tree_collapse").hide();
            $("#lb_tree_expand").show();
