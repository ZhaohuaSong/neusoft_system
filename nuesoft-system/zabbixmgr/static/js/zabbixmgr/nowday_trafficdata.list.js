...
//  Datatables 初始化页面 关键绑定 ID (dynamic-table)
    ...
    first_url = "/zabbixmgr/nowdayanalysis/list";
    console.info(cru_url)
...
// 查询方法
        $("#btn-traffic_search").bind("click",
            function () {
                ...
                begindate = args2.match(/(\d+)/g)[0];
                lastdate = args3.match(/(\d+)/g)[0];
                ...
                var arg = t + '/nowdaytrafficdata/list/' + makeupCo + '/' + makeupCo_1 + '/' + begindate + '/' + lastdate + '/' + args1 + '/' + industry_id;
