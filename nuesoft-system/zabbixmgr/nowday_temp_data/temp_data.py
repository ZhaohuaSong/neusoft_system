...
class CreateTempDatas(PublicWays):
    ...
    def time_interval(self):
        ...
        today_time = int(time.mktime(time.strptime(str(datetime.date.today()), '%Y-%m-%d')))
        TGHistoryUint1Min.objects.filter(clock__lt=today_time).delete()
        TGHistoryUint5Min.objects.filter(clock__lt=today_time).delete()
        TGHistoryUint1Hour.objects.filter(clock__lt=today_time).delete()
        try:
            sl = TGHistoryUint1Min.objects.all().count()
            pre_time = TGHistoryUint1Min.objects.all()[sl-1].clock + 60
        except:
            pre_time = int(time.mktime(time.strptime(str(datetime.date.today()), '%Y-%m-%d')))
        last_time = int(time.time()) - ( int(time.time()) % 3600 )
        self.data_num = (last_time-pre_time)/60
    ...
    def onemin_data(self, qs):
        ...
        for k in range(len(self.incoming_interface_list)):
            ...
            out_group_list = []
            for i in range(len(in_li)):
                in_val = list(HistoryUint.objects.using('zabbixdb').filter(Q(itemid=in_li[i]) & Q(clock__gte=self.ptime) & Q(clock__lt=self.ltime)).values_list('value', flat=True))
                out_val = list(HistoryUint.objects.using('zabbixdb').filter(Q(itemid=out_li[i]) & Q(clock__gte=self.ptime) & Q(clock__lt=self.ltime)).values_list('value', flat=True))
                ck_in = list(HistoryUint.objects.using('zabbixdb').filter(Q(itemid=in_li[i]) & Q(clock__gte=self.ptime) & Q(clock__lt=self.ltime)).values_list('clock', flat=True))
                ck_out = list(HistoryUint.objects.using('zabbixdb').filter(Q(itemid=out_li[i]) & Q(clock__gte=self.ptime) & Q(clock__lt=self.ltime)).values_list('clock', flat=True))


                if len(in_val) == 0:
                    in_val = [0 for i in range(self.data_num)]
                # elif len(in_val) != self.data_num:
                else:
                    in_val_new = []
                    f = 0
                    vv = ck_in[0]
                ...
                if len(out_val) == 0:
                    out_val = [0 for i in range(self.data_num)]
                # elif len(out_val) !=self.data_num:
                else:
                    out_val_new = []
                    f = 0
                    vv = ck_out[0]
            ...
            for i in range(len(in_group_list[0])):
                ...
                in_inter = TGHistoryUint1Min(itemid=in_clientid, clock=ti, value=in_value, ns=int(in_args.keys()[0]), industry_id=industry_id)
                out_inter = TGHistoryUint1Min(itemid=out_clientid, clock=ti, value=out_value, ns=int(out_args.keys()[0]), industry_id=industry_id)
                interval.append(in_inter)
                interval.append(out_inter)
                ti += 60
            TGHistoryUint1Min.objects.bulk_create(interval)
            print k
    ...
    def group_history_5min_data(self):
        ...
        for i in range(len(in_itemid)):
            ti = self.ptime
            cltgroup = []
            in_list = list(TGHistoryUint1Min.objects.filter(Q(itemid=in_itemid[i]) & Q(clock__gte=self.ptime) & Q(clock__lt=self.ltime)).values_list('value', flat=True))
            out_list = list(TGHistoryUint1Min.objects.filter(Q(itemid=out_itemid[i]) & Q(clock__gte=self.ptime) & Q(clock__lt=self.ltime)).values_list('value', flat=True))
            ...
            for j in range(len(in_list)/5):
                ...
                in_val = TGHistoryUint5Min(itemid=in_itemid[i], clock=ti, value_min=in_min_val, value_max=in_max_val, value_avg=in_avg_val, value=in_list[j*5+4], industry_id=industry_id)
                out_val = TGHistoryUint5Min(itemid=out_itemid[i], clock=ti, value_min=out_min_val, value_max=out_max_val, value_avg=out_avg_val, value=out_list[j*5+4], industry_id=industry_id)
                cltgroup.append(in_val)
                cltgroup.append(out_val)

                ti += 60*5

            TGHistoryUint5Min.objects.bulk_create(cltgroup)
            print i
    ...
    def group_history_data(self, time_interval):
        ...
        if time_interval == 12:
            get_model = TGHistoryUint5Min
            set_model = TGHistoryUint1Hour
            interval = 60
        elif time_interval == 24:
            get_model = TGHistoryUint1Hour
            set_model = TGHistoryUint1Day
            interval = 1440
